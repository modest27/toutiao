<template>
  <div class="article-container">
    <!-- 导航栏 -->
    <van-nav-bar class="page-nav-bar" left-arrow title="未来头条" @click-left="onClickLeft"></van-nav-bar>
    <!-- /导航栏 -->
    ...
    <div class="main-wrap">
      ...
      <div class="article-detail" v-else-if="article.title">
        ...
        <van-cell class="user-info" center :border="false">
          ...
          <UserFollow :isFollow="article.is_followed" :userId="article.aut_id" class="follow-btn" @update-follow="article.is_followed= $event"></UserFollow>
        </van-cell>
        <!-- /用户信息 -->
        ...
        <!-- 评论列表区域 -->
        <CommentList :source="article.art_id" :list="commentList" @onload-success="totalCommentCount=$event.total_count" @reply-click="onReplyClick"></CommentList>
        <!-- /评论列表区域 -->
    ...
    <van-popup v-model="isReplyShow" style="height:100%" position="bottom">
      <!-- 弹出层是懒渲染:只有在第一次加载时才会渲染里面内容，请求数据，所以这里要v-if销毁组件 -->
...
export default {
  ...
  data() {
    return {
      ...
      isPostShow: false, // 控制发布评论弹出层展示状态
      commentList: [], //评论列表数据
      isReplyShow: false, //控制回复评论弹出层展示状态
  ...
  methods: {
    async loadArticle() {
      ...
      this.loading = false //无论成功还是失败,都要关闭 loading 状态
    },
    onClickLeft() {
      this.$router.back()
    },
    ...
    onPostSuccess(data) {
      this.isPostShow = false //发布成功，关闭弹出层
      this.commentList.unshift(data.new_obj)
    },
    onReplyClick(cmt) {
      this.currentComment = cmt
      //展示评论回复弹出层
